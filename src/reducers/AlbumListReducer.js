import {
  ...
  FETCH_ALBUM_SUCCESS,
  FETCH_ALBUM_ERROR,
  INITIALIZE_ALBUM_LIST,
} from '../actions/AlbumListAction';
...
const AlbumListReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case FETCH_ALBUM_SUCCESS:
      console.log('New album list:', action.payload.content);

      const newAlbumList = [...state.albumList, ...action.payload.content];
      const lastAlbumId =
        action.payload.content[action.payload.content.length - 1].albumId;
      const first = false;
